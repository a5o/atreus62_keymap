...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	OSL(_GREEK), OSL(_FUN), KC_LGUI, KC_LALT, RAISE, KC_SPC, KC_BSPC, KC_ENT, KC_SPC, LOWER, KC_LEFT, KC_DOWN, KC_UP, KC_RIGHT
	),
	...
	KC_TRNS, XP(LTHET,UTHET), XP(LOMEG,UOMEG), XP(LEPSI,UEPSI), XP(LRHO,URHO), XP(LTHET,UTHET), XP(LTAU,UTAU), XP(LUPSI,UUPSI), XP(LIOTA,UIOTA), XP(LOMIC,UOMIC), XP(LPI,UPI), KC_NO, 
	KC_TRNS, XP(LALPH,UALPH), XP(LSIGM,USIGM), XP(LDELT,UDELT), XP(LPHI,UPHI), XP(LGAMM,UGAMM), XP(LETA,UETA), XP(LXI,UXI), XP(LKAPP,UKAPP), XP(LLAMB,ULAMB), KC_NO, KC_NO, 
	KC_TRNS, XP(LZETA,UZETA), XP(LCHI,UCHI), X(FSIGM), KC_NO, XP(LBETA,UBETA), XP(LNU,UNU), XP(LMU,UMU), X(LTEQ), X(GTEQ), X(NOTEQ), X(PLMIN), 
